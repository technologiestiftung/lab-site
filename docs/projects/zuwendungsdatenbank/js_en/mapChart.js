var mapChart = function(_container, _geojson, _data, _count_data, _dict, _filterFunction, _filterKey, _tooltip, _isMini, _theYear){
  ...
  var module = {},
    ...
    toggle_sum = toggle_text.append('tspan').text('Sum (€)').attr('class', 'toggle sum').on('click', ()=>{ 
      display_mode = 'sum';
      module.update(); 
    }),
    toggle_sep1 = toggle_text.append('tspan').text(' / '),
    toggle_count = toggle_text.append('tspan').text('Count').attr('class', 'toggle count').on('click', ()=>{ 
      display_mode = 'count'; 
      module.update(); 
    }),
    toggle_sep2 = toggle_text.append('tspan').text(' / '),
    toggle_ratio = toggle_text.append('tspan').text('Ratio Sum/Count').attr('class', 'toggle ratio').on('click', ()=>{ 
      display_mode = 'ratio';
      module.update();
    }),
    display_mode = 'sum',
  ...
  module.updateToolTip = (x,y,d,di)=>{
    ...
    tooltip.show({
      title:(dict_keys[d.key]==9999999)?`Outside Berlin`:`PLZ-${dict_keys[d.key]}`,
      body: `Sum in €:<br /><i>${currency(d.value)}` + ((showPercentage) ? `&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;${(d.value/all.value()*100).toFixed(2)}%`: '') + `</i><br /><br />Number of grants:<br /><i>${count_data.all()[di].value}`+ ((showPercentage)? `&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;${(count_data.all()[di].value/all_groups.value()*100).toFixed(2)}%` : '') + `</i>`,
      x:x,
      y:y
    })
  }
