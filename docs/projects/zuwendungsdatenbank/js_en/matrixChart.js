var matrixChart = function(_container, _labels, _data, _dict, _count, _filterFunction, _filterKey, _tooltip){
  ...
  module.updateToolTip = (x,y,d) => {
    ...
    let tableStr = '<tr><th>Year</th><th>Sum</th><th>Count</th></tr>'
    ...
    tooltip.show({
      title:d.label,
      body:`Sum in (€):<br /><i>${currency(d.sum)}`+ ((showPercentage)? `&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;${(d.sum/all.value()*100).toFixed(2)}%`: '') + `</i><br><br />Number of grants:<br /><i>${d.count}`+ ((showPercentage)? `&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;${(d.count/all_groups.value()*100).toFixed(2)}%`: '') + `</i><br /><br /><table>${tableStr}</table>`,
      x:x,
      y:y
    });
  }
  ...
  module.init = function(){
    ...
    sortButton.append('tspan').text('Sum').classed('sort-sum',true).on('click',function(){ module.toggleSort('sum'); });
    sortButton.append('tspan').text(' / ');
    sortButton.append('tspan').text('↑').classed('sort-d-count',true);
    sortButton.append('tspan').text('↓').classed('sort-d-icount',true);
    sortButton.append('tspan').text('count').classed('sort-count',true).on('click',function(){ module.toggleSort('count'); });
