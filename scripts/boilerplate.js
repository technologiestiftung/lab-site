...
if (fs.existsSync('./authors/' + default_params.AUTHOR + '.json')) {
    let author = JSON.parse(
        fs.readFileSync('./authors/' + default_params.AUTHOR + '.json', 'utf8')
    );
...
if (default_params.PROJECT == '{{PROJECT}}') {
    console.log(
        'You must provide the PROJECT param, in order to create a new folder for your project.'
    );
} else {
    ...
    if (fs.existsSync(path)) {
        console.log(
            'PROJECT folder already exists, overwriting is disabled, please delete the folder first'
        );
    } else {
        //Create Folder
        fs.mkdirSync(path);
        ...
        //Copy Boilerplate
        ncp(
            './docs/projects/boilerplate' +
                (default_params.TYPE == 'full' ? '' : '-light'),
            path,
            function(err) {
                if (err) {
