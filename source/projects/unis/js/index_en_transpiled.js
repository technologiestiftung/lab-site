...
var mapChart = function mapChart(_data, _geojson, _filterFunction, _filterKey, _container, _proj, _type) {
  ...
  pointer = overlay.append('img').attr('src', '../images/zoom.png').attr('width', 54).attr('height', 54);
  hint = overlay.append('p').attr('class', 'hint');
  ...
  function freeZoom() {
    ...
    if (scaleObj.scale > 50000 && type == 'GER') {// console.log(svgMap);
      //svgMapGer[0].childNodes[0].childNodes[2].childNodes[0].classList.add('hide')
    } else if (scaleObj.scale < 40000 && type == 'GER') {//svgMapGer[0].childNodes[0].childNodes[2].childNodes[0].classList.remove('hide')
    }

    if (scaleObj.scale > 350000 && type == 'BER') {//svgMapBer[0].childNodes[0].childNodes[2].childNodes[0].classList.add('hide')
    } else if (scaleObj.scale < 200000 && type == 'BER') {//svgMapBer[0].childNodes[0].childNodes[2].childNodes[0].classList.remove('hide')
    }
...
var beeChart = function beeChart(_data, _filterFunction, _filterKey, _container, _type) {
  ...
  hint = overlay.append('p').attr('class', 'hint');
  hint.text("Mark a vertical selection to \n select mupltiple universities.");
  ...
  module.refresh = function (id, event) {
    ...
    var radius = event == 'mouseover' ? selection.transition().duration(100).attr('r', '6px') : selection.transition().duration(100).attr('r', '2px');
  };
...
var brushedTooltip = function brushedTooltip(_data, _container, _type) {
  ...
  module.init = function () {
    ...
    container1.append('div').attr('class', 'tt-subheadline').text('ø Age');
    avgAge = container1.append('div').attr('class', 'tt-value').text("Select area");
    container2 = subWrapper.append('div').attr('class', 'tt-wrapper');
    container2.append('div').text('Students').attr('class', 'tt-subheadline');
    container3 = subWrapper.append('div').attr('class', 'tt-wrapper');
    sumStudentsWrapper = container2.append('div').attr('class', 'tt-value').text("Select area");
    ageSubline = container1.append('div').attr('class', 'tt-subline').text("for more information");
    studentsSubline = container2.append('div').attr('class', 'tt-subline').text("".concat(0, "% of 2.74 mio. students."));
    container3.append('div').attr('class', 'tt-subheadline').text('ø studies');
    avgStudiesWrapper = container3.append('div').attr('class', 'tt-value').text("Select area");
    studiesSubline = container3.append('div').attr('class', 'tt-subline').text("von 2\u2013350 studies.");
  };
  ...
  module.update = function (selection) {
    ...
    var sumStudentsPercent = round(sumStudents / 2670000) * 100;
    ageSubline.text("of selected universities.");
    studentsSubline.text("".concat(sumStudentsPercent, "% of 2.74 mio. students."));
    headline.text("".concat(round(selection.length), " of ").concat(countUnis, " universities selected."));
    avgAge.text(function (d) {
      ...
      return "".concat(round(avgUniAge).toString().replace('.', ','), " years");
    });
    ...
    avgStudiesWrapper.text(function (d) {
      ...
      return "".concat(round(avgStudies).toString().replace('.', ','), " years");
    });
  };
...
var tooltip = function tooltip(_data, _container) {
  ...
  module.init = function () {
    ...
    container2 = subWrapper.append('div').attr('class', 'tt-wrapper');
    container3 = subWrapper.append('div').attr('class', 'tt-wrapper');
    ageSubheadline = container1.append('div').attr('class', 'tt-subheadline').text('Founded');
    age = container1.append('div').attr('class', 'tt-value').text('Select university');
    ageSubline = container1.append('div').attr('class', 'tt-subline').text('for more information');
    var studentsSubheadline = container2.append('div').attr('class', 'tt-subheadline').text('Students');
    students = container2.append('div').attr('class', 'tt-value').text('Select university');
    studentsSubline = container2.append('div').attr('class', 'tt-subline').text('for more information');
    studiesSubheadline = container3.append('div').attr('class', 'tt-subheadline').text('Studies');
    studies = container3.append('div').attr('class', 'tt-value').text('Select university');
    studiesSubline = container3.append('div').attr('class', 'tt-subline').text("#".concat(12, " of 374"));
  };
  ...
  module.update = function (data) {
    ...
    ageSubline.text("#".concat(data.rAge, " of 374"));
    studentsSubline.text("#".concat(data.rStudents, " of 374"));
    studiesSubline.text("#".concat(data.rStudies, " of 374"));
...
var table = function table() {
  ...
  module.updateBerlin = function (filterCurrent, filteredData) {
    var tableItem = tableItemsBerlin.selectAll('div').data(filteredData).enter().append('div').attr('class', 'table-item').on('click', function () {
    ...
    var tableContent = tableItem.append('div').attr('class', 'td-content');
    var tableItemValue = tableItem.append('div').attr('class', 'td-value').text(function (d, i) {
      return d[filterCurrent];
    });
    var tableItemHeadline = tableContent.append('div').attr('class', 'td-headline').text(function (d) {
      return d.name;
    });
    var tableItemSubline = tableContent.append('div').attr('class', 'td-subline');
    var col1 = tableItemSubline.append('div').attr('class', 'table-col col1');
    var col2 = tableItemSubline.append('div').attr('class', 'table-col col2');
    var col3 = tableItemSubline.append('div').attr('class', 'table-col col3');
    tableItemCounty = col1.append('div').attr('class', 'td-county').text(function (d) {
      return d.county;
    });
    var tableItemStudies = col2.append('div').attr('class', 'td-studies').text(function (d) {
      return "".concat(d.count_studies, " Studies");
    });
    var tableItemYear = col1.append('div').attr('class', 'td-year').text(function (d) {
      return "".concat(d.year, " gegr\xFCndet");
    });
    var tableItemStudents = col2.append('div').attr('class', 'td-students').text(function (d) {
      return "".concat(d.count_students, " Studierende");
    });
    var tableItemType = col3.append('div').attr('class', 'td-sponsor').text(function (d) {
      return d.sponsor;
    });
  };

  module.update = function (filterCurrent, filteredData) {
    var tableItem = tableItems.selectAll('div').data(filteredData).enter().append('div').attr('class', 'table-item').on('click', function (d) {
      d3.select(this).classed("active", d3.select(this).classed("active") ? false : true);
    });
    var tdIndex = tableItem.append('div').attr('class', 'td-index').text(function (d, i) {
      return i + 1;
    });
    var tableContent = tableItem.append('div').attr('class', 'td-content');
    var tableItemValue = tableItem.append('div').attr('class', 'td-value').text(function (d, i) {
    ...
    var col2 = tableItemSubline.append('div').attr('class', 'table-col col2');
    var col3 = tableItemSubline.append('div').attr('class', 'table-col col3');
    var tableItemCounty = col1.append('div').attr('class', 'td-county').text(function (d) {
      return d.county;
    });
    var tableItemStudies = col2.append('div').attr('class', 'td-studies').text(function (d) {
      return "".concat(d.count_studies, " Studies");
...
d3.queue().defer(d3.json, "../data/unis.json").defer(d3.json, "../data/merged.topojson").defer(d3.json, "../data/berlin.topojson").await(function (error, unis, germany, berlin) {
  ...
  var public1 = unis_berlin.filter(function (uni) {
  ...
  map_chart.init();
  var map_chart_legend = legend(d3.select('#legend-bars'));
