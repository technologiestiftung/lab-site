class Radarchart {
    ...
    createTitle() {
        d3.json('/projects/bikerides/assets/names_dict.json').then(data => {
    ...
    updateCircles(category, color) {
        ...
        this.circles[category]
            ...
            .attr('cx', (d,i) => {
                ...
                let polar_coord_x = (this.width / 2) * (d[this.category] / this.max_value) * this.factor * Math.sin(i*this.radians / this.total);
                let polar_coord_y = (this.height / 2) * (d[this.category] / this.max_value) * this.factor * Math.cos(i*this.radians / this.total);
                
                let single_coord = [];
                single_coord.push(polar_coord_x);
                single_coord.push(polar_coord_y);
                this.node_coords[this.category].push(single_coord);
