...
d3.select('#btn-toggle').on('click', function(){
  if(!toggleState){
      ...
      d3.select(this).property('value', 'Zurück zum Korrektur-Interface')
  }else{
      toggleState = false;
      d3.select('#field-template').style('display','none');
      d3.select('#table-container').style('display','block');
      d3.select(this).property('value', 'Template-JSON anzeigen')
  }
});
...
function refreshCSV(){
  d3.select('#btn-refresh-message').text('Daten werden importiert...');
...
function tableHead(sel, type){
  ...
  tr.append('th').text('Wert');
  tr.append('th').text('Anzahl');
  tr.append('th').text('Ersetzen durch');
  tr.append('th').text('Gehört nicht zur Gruppe');
}
...
function buildTable(template){
  ...
  if(cleanTemplate.length > 0){
      ...
      table.append('tbody').append('tr').append('td').attr('colspan',3).text('Es wurden keine ähnlichen Texte gefunden.');
  }
...
d3.selectAll('.btn-send').on('click', function(){
  ...
  if(!analyse_state){
      ...
      msg.text('Daten werden bearbeitet...');
      ...
      values.forEach(function(d,i){
          ...
          request[d[0]] = (d[1])?o.property('value'):o.node().value;
      });
