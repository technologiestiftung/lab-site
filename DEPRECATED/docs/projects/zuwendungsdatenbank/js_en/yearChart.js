var yearChart = function(_container, _dates, _counts, _filterFunction, _filterKey, _tooltip){
  ...
  var module = {},
    ...
    yAxis = d3.axisLeft().ticks(10).scale(y).tickFormat(function(d){
      if(d >= 1000000000){
        return (d/1000000000).toFixed(2) + ' Bil. €';
      }else if(d >= 1000000){
    ...
    x_label = g.append('text').text('Sum (€)').style('font-weight','bold').attr('text-anchor','end'),
    x_right_label = g.append('text').text('count').style('font-weight','bold'),
  ...
  function updateToolTip(x,y,d){
    ...
    tooltip.show({
      title:d.count.key,
      body:`Sum in €:<br /><i>${currency(d.date.value)}`+ ((showPercentage)? `&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;${(d.date.value/all.value()*100).toFixed(2)}%`: '') + `</i><br><br />Number of grants:<br /><i>${d.count.value}`+ ((showPercentage)? `&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;${(d.count.value/all_groups.value()*100).toFixed(2)}%`: '') + `</i>`,
      x:x,
      y:y
    });
  }
